...
Api.prototype.addActivity = function(activity, callback) {
	...
	Flow.exec(
			...
			function(err, profile) {
				...
				var items = [];
				items.push(activity);
				Ostatus.as.render(host, items, profile, this);
			},
...
Api.prototype.push = function(id, body, signature, callback) {
	var subscription;
	Flow.exec(
			...
			function(err, sub) {
				...
				Ostatus.as.parseFeed(body, this);
			},
			function(err, activities) {
				if (err) return callback(err);
				if (activities.items) {
					for (i=0;i<activities.items.length; i++) {
						...
						activity._key = "message:" + subscription.user + ":" + activity.id;
...
Api.prototype.message = function(username, body, callback) {
	var id;
	Flow.exec(
		function() {
			Database.get("user:" + username, this);
		},
		function(err, user) {
			if (err) return callback(err);
		    console.log(user);
		    Ostatus.salmon.unpack(body, this);
		},
		function(err, verified, activity) {
			if (err) return callback(err);
			console.log("Signature verified: " + verified);
			if (!verified) return callback(new Error("Could not verify signature")); 
			console.log("Activity: " + activity.title);
			activity._type = "message";
			activity._key = "message:" + username + ":" + activity.id;
			Database.add(activity, this);
		},
